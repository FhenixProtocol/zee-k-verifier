...
The image is signed. The build signs the pushed digest with keyless Cosign and attests SLSA build provenance, both logged in the public Rekor log. The signature does **not** gate the runtime — the digest pinned in each partner's CEL is still the whole enforcement. It gates the **pin**: before a partner pins a new digest, it runs `cosign verify` on its own machine and asserts the exact digest and the exact commit. A non-zero exit means it does not pin. The build's job summary prints that command with every value filled in.
